...
class CBUtils: NSObject {
    ...
    class func caloriesFrom(step: Int) -> Float {
        ...
        return caloriesFrom(isMale: UserDefaults.bool(of: .isMale), ageYear: year, weightKG: weight, heightCM: height, stepWalk: step, stepRun: 0, now: Date())
    }
    
    class func caloriesFrom(isMale: Bool, ageYear: Int, weightKG: Int, heightCM: Int, stepWalk: Int, stepRun: Int, now: Date) -> Float {
        ...
        // the above is BMR per day, we need BMR till now.
        bmr *= Float(Calendar.current.component(.hour, from: now)) / 24.0
